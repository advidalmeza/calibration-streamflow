...
#' @param day day
#' @param year  year
#' @param high_flow_months which to use default (April)
...
compute_highflowmetrics <- function(m, o, month, day, year, 
  ...
  tmp <- flow %>% group_by(wy) %>% 
    summarize(max_o = max(o), max_m = max(m))
  
  annual_max_err <- mean(tmp$max_m - tmp$max_o)
  
  annual_max_cor <- cor(tmp$max_m, tmp$max_o)
  
  if (is.null(max_err_annual_max)) {
    max_err_annual_max <- 0.5*mean(tmp$max_o) 
    }
